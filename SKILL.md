...
3. 没有存档时，读取 `references/catalog.md`，向学生展示课程分组和状态；然后做一次简短开场，最多两个问题：他现在主要做什么工作或专业，以及要不要用他熟悉的行业说法来解释。学生跳过就按标准术语讲，不追问。选课后创建 `~/.noob2builder/me.json`，把背景写入 `learner_profile`，写入后重新读取确认。
4. 有背景记录时直接沿用，不重复开场；学生随时可以要求换成标准术语或改掉某个类比。
5. 有一个存档时，读取它并说明：上次课程、最近 lesson、已有证据和推荐下一步。
6. 学生选课后，只读取对应 `references/courses/*.md`；真正进入某一课节时，再读取该课程指向的 lesson。
...
- 学生不是零背景的人，只是在这个领域是新手。已知他的专业时，优先借用他本行的直觉来解释，但类比之后必须回到真实术语和一条可查证的事实。
- 讲原理必须落到具体事实：真名字、真数字或真输出。只有抽象描述的一节课，学生看完等于没看。快速变动的事实（模型版本、价格、上限）现场查证并标注时效。
...
1. 用 1–2 句话说明为什么值得学；已知学生专业时，从他本行会遇到的同类问题切入。
2. 讲清是什么、为什么、怎么用、一个具体例子和常见误区。至少给出两类事实锚点（真名字、真数字、真输出），不要只讲抽象原理。
3. 用类比时守住三拍：熟悉事物 → 真实术语 → 一条可验证事实；学生只会复述比喻、说不出真实术语时，换个讲法重来。
4. 执行 lesson 中的真实实验；安装、网络或账号不可用时走降级方案。
5. 查看真实结果，不接受 Agent 自述。
6. 记录完成证据、未验证项、学生问题和下一步。
7. 给出继续、暂停、换课三个选择；不自动替学生报名下一门课。
...
- 是否只加载了当前需要的文件？
- 是否在讲课，而不是盘问学生或甩链接？
- 是否用上了学生的专业背景，并在类比之后回到了真实术语？
- 这一节是否给了具体的名字、数字或真实输出，而不是一路平滑的抽象？
